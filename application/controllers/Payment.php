...
class Payment extends CI_Controller {
	...
	public function squarePaymentProcess()
	{
		if($_SERVER['REQUEST_METHOD'] === 'POST') 
		{
			...
			//$access_token = 'EAAAELHWzPy0_vxLlgwV7Sjw7ejq0obyu6DRBv79nVFA469xLfRw-_8UXq0U5P3g';
			...
			//$location_id = 'CBASEOkESK5R0mtyyL7hRKde7GwgAQ ';
			$nonce = $_POST['nonce'];
			...
			$amountMoney->setAmount($totalPayment*100);
			$amountMoney->setCurrency("AUD");
			...
			try {
					...
					$set=array('payment_id'=>$paymentId,
							 ...
							 'date'=>$date,
							 'payment_result'=>addslashes($result),
							 'new_flag'=>'0');
			...
			<div style="background:#fff; -webkit-box-shadow: 0px 0px 20px 0px rgba(0,0,0,0.21); -moz-box-shadow: 0px 0px 20px 0px rgba(0,0,0,0.21); box-shadow: 0px 0px 20px 0px rgba(0,0,0,0.21);">
				...
				//echo $error;
					...
					$set=array('payment_id'=>$paymentId,
							 ...
							 'amount'=>addslashes($totalAmount),
							 'date'=>$date,
							 'payment_result'=>addslashes($e),
							 'new_flag'=>'0');
		
					$this->contact_model->insertContactUs("pay_error",$set);
