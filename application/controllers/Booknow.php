...
$pick_up_date_raw = addslashes($this->input->post('pick_up_date'));

// Use DateTime to parse the date with the expected format
$dateTime = DateTime::createFromFormat('d/m/Y', $pick_up_date_raw);

// Check if the date is valid before converting it
if ($dateTime) {
    // Convert DateTime to string in the desired format
    $pick_up_date = $dateTime->format('Y-m-d');
} else {
    // Handle invalid date format
    $pick_up_date = null; // or handle it as needed
}
...
// Additional fields for boolean checkboxes
$rear_facing_seat = $this->input->post('rear_facing_seat') ? 1 : 0;
$forward_facing_seat_harness = $this->input->post('forward_facing_seat_harness') ? 1 : 0;
$booster_seat = $this->input->post('booster_seat') ? 1 : 0;
$surfboards = $this->input->post('surfboards') ? 1 : 0;
$golf_clubs = $this->input->post('golf_clubs') ? 1 : 0;
$skis = $this->input->post('skis') ? 1 : 0;
$snowboards = $this->input->post('snowboards') ? 1 : 0;
$prams = $this->input->post('prams') ? 1 : 0;
$bike_cases = $this->input->post('bike_cases') ? 1 : 0;
$other_items = $this->input->post('other_items') ? 1 : 0;

// Additional field for double
$total_quotation = isset($_POST['total_quotation']) ? (double)$_POST['total_quotation'] : 0.0;

$date = date('Y-m-d');

// Additional fields for stops
$stops = array();
for ($i = 1; $i <= 6; $i++) {
    $stop_key = 'departure_stop_' . $i;
    $stops[] = addslashes($this->input->post($stop_key));
}

// Remove empty values
$stops = array_filter($stops);

// Combine stops with line breaks and commas
$other_destination .= implode(",\n", $stops);
...
$set = array(
    ...
    'pick_up_date' => $pick_up_date, // Store the date as a string
    ...
    'flight_no' => $flight_no,
    'new_flag' => 0,
    'rear_facing_seat' => $rear_facing_seat,
    'forward_facing_seat_harness' => $forward_facing_seat_harness,
...
echo '<script>';
echo 'console.log(' . json_encode($set) . ');';
echo '</script>';
...
try {
    $insertSuccess = $this->booknow_model->insertValue("booking_departure", $set);
    ...
    //       if($status['success']){ 
			...
			$departure_type=addslashes($this->input->post('departure_type')); 
			$fname=addslashes($this->input->post('Arrival'));
			$lname=addslashes($this->input->post('lnameArrival'));
			$name=$fname." ".$lname;
			$email=addslashes($this->input->post('emailArrival'));
			$phone=addslashes($this->input->post('phoneArrival'));
			$pick_up_time=addslashes($this->input->post('pick_up_timeArrival'));
			$pick_up_date_raw = addslashes($this->input->post('pick_up_dateArrival'));
...
if ($dateTime) {
    ...
    $pick_up_date = null; // or handle it as needed
			...
			$destination=addslashes($this->input->post('destinationArrival'));
			
			$pick_up_address=addslashes($this->input->post('pick_up_addressArrival'));
			$adult=addslashes($this->input->post('adultArrival'));
			$children=addslashes($this->input->post('childrenArrival'));
			$infant=addslashes($this->input->post('infantArrival'));
			$post_code=addslashes($this->input->post('post_codeArrival'));
			
			$child_age=addslashes($this->input->post('child_age'));
			$infant_age=addslashes($this->input->post('infant_age'));
			$client_message=addslashes($this->input->post('client_messageArrival'));
			...
			// Additional fields for boolean checkboxes
$rear_facing_seat = $this->input->post('rear_facing_seatArrival') ? 1 : 0;
$forward_facing_seat_harness = $this->input->post('forward_facing_seat_harnessArrival') ? 1 : 0;
$booster_seat = $this->input->post('booster_seatArrival') ? 1 : 0;
$surfboards = $this->input->post('surfboardsArrival') ? 1 : 0;
$golf_clubs = $this->input->post('golf_clubsArrival') ? 1 : 0;
$skis = $this->input->post('skisArrival') ? 1 : 0;
$snowboards = $this->input->post('snowboardsArrival') ? 1 : 0;
$prams = $this->input->post('pramsArrival') ? 1 : 0;
$bike_cases = $this->input->post('bike_casesArrival') ? 1 : 0;
$other_items = $this->input->post('other_itemsArrival') ? 1 : 0;

// Additional field for double
$total_quotation = isset($_POST['total_quotationArrival']) ? (double)$_POST['total_quotationArrival'] : 0.0;
